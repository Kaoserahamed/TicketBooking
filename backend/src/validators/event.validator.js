...
const listEventsQuerySchema = z.object({
  ...
  status: z.enum(PUBLIC_EVENT_STATUSES, { message: 'status must be PUBLISHED or ACTIVE' }).optional(),
  ...pagingFields,
});
const eventIdParamSchema = z.object({ id: z.coerce.number().int().positive('id must be a positive integer') });
const listShowsQuerySchema = z.object({ ...pagingFields });
const url = z.string().trim().max(500, 'posterUrl must be at most 500 characters');
const posterUrl = z.union([url.min(1, 'posterUrl must not be empty'), z.literal(''), z.null()]).optional();
...
const createEventSchema = z.object({
  ...
  posterUrl,
  status: z.enum(EVENT_STATUSES, { message: `status must be one of: ${EVENT_STATUSES.join(', ')}` }).optional(),
});
...
module.exports = { EVENT_STATUSES, PUBLIC_EVENT_STATUSES, listEventsQuerySchema, eventIdParamSchema, listShowsQuerySchema, createEventSchema, updateEventSchema, listAdminEventsQuerySchema };
