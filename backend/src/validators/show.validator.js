...
const dateTime = z
  .string({ message: 'must be a datetime string' })
  .trim()
  .min(1, 'must not be empty')
  .refine((v) => !Number.isNaN(new Date(v).getTime()), { message: 'must be a valid datetime' });
...
const listShowsQuerySchema = z.object({
  ...
  status: z
    .enum(SHOW_STATUSES, { message: `status must be one of: ${SHOW_STATUSES.join(', ')}` })
    .optional(),
...
const createShowSchema = z.object({
  ...
  defaultPrice: z
    .number()
    .min(0, 'defaultPrice must be >= 0')
    .max(1000000, 'defaultPrice is too large')
    .optional(),
});
