...
describe('AdminEventsPage', () => {
  it('renders the admin event table and creates an event', async () => {
    ...
    expect(adminApiMock.listEvents).toHaveBeenCalledWith(
...
describe('AdminBookingsPage', () => {
  it('lists bookings with customer info and applies the status filter', async () => {
    ...
    expect(adminApiMock.listBookings).toHaveBeenCalledWith(
      expect.objectContaining({ limit: 10, offset: 0 })
    )

    await userEvent.selectOptions(
      screen.getByRole('combobox', { name: 'Filter by booking status' }),
      'CONFIRMED'
    )
    await userEvent.click(screen.getByRole('button', { name: 'Apply filters' }))
    await waitFor(() =>
      expect(adminApiMock.listBookings).toHaveBeenLastCalledWith(
